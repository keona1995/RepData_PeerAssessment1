...
```
The average number of steps taken per day is `r round(avgdailysteps,2)`.
The median number of steps taken per day is `r mediandailysteps`.
...
```
The maximum number of steps taken on average over the entire period occurs at the `r (maxperiod -1)*5` minute mark, approximately `r round(approxtime,2)`  hours after midnight.
...
Before imputing values, the number of observations with missing values was `r totalNAs`.
The effect of imputing data was to raise the mean of the step count per day. In the first analysis where missing values were ignored, the mean was `r round(avgdailysteps,2)`. In this second analysis with imputed values, the mean was about `r as.integer(avgdailystepsimputed)`. The median was the same in both analyses, with a value of `r as.integer(mediandailystepsimputed)`.
